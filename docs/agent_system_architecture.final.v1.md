...
本项目采用“读 -> 用 -> 写”的记忆闭环，但不把所有内容都做成向量记忆。文章中的四类记忆映射到本项目如下：

| 记忆类型 | 本项目内容 | 生命周期和存储 | 使用方式 |
| --- | --- | --- | --- |
| 感知记忆 | 当前用户消息、上传内容和本次请求元数据 | 单次请求；不写入长期记忆，除非被提炼为有价值事实 | 作为本次运行输入 |
| 短期记忆 | 当前会话消息、工具观察、范围、假设和待解决问题 | 会话期间在 context 中使用；超限时压缩为结构化摘要 | 直接注入当前 Agent run |
| 长期情节记忆 | 已完成分析的摘要、用户确认过的训练目标、历史结论及其证据引用 | PostgreSQL 持久化；按用户隔离，并带时间、来源、状态和过期信息 | 会话开始主动加载，复杂问题按需检索 |
| 实体记忆 | 玩家、地图、英雄、训练目标、分析窗口和可验证行为事实 | PostgreSQL 结构化表；使用精确查询，不依赖向量相似度 | 按当前用户、范围和实体精确读取 |
| 通用语义/程序知识 | 审核后的教练原则、地图规则、训练 Drill 和产品 SOP | 独立 Knowledge Store；文本可做关键词/向量混合检索 | 仅在需要通用解释时按需检索 |

这里的“三层实现”是：

```text
Core：当前用户画像、当前目标和已确认范围（结构化、少量、始终可见）
Recall：当前会话历史和历史分析摘要（按时间或相关性召回）
Archive：审核知识和可追溯历史材料（主动检索，不默认塞进上下文）
```

玩家比赛事实不属于模型自由生成的长期记忆：它必须继续由 PostgreSQL 中的原始数据、确定性指标、分析窗口和回合证据提供。历史分析摘要只能引用这些稳定证据 ID，不能升级为无证据的用户标签。通用知识也不能制造玩家事实。

### 5.1 写入规则

只写入“下次任务知道它会明显更有帮助”的内容：

- 用户明确确认的偏好、训练目标和范围；
- 经过确定性分析并带证据 ID、样本量和时间范围的观察；
- 通过审核、带来源定位和补丁版本的通用知识。

不写入原始工具 JSON、隐藏思维过程、无证据推断、一次性闲聊或未审核的模型总结。每条可更新记忆必须带 `userId`（通用知识除外）、`sourceType`、`sourceId`、`createdAt`、`updatedAt`、`validFrom`、`validTo`、`confidence` 和 `status`。新事实与旧事实冲突时保留历史记录，将旧记录标为 `superseded`，不能静默覆盖。

### 5.2 读取规则

每次会话开始先读取 Core：认证用户、已确认训练目标、当前范围和未完成问题；再根据用户问题读取少量 Recall。只有问题需要通用打法或训练方法时，才调用知识检索工具。检索必须先做用户/范围/补丁/审核状态过滤，再做关键词或向量排序，并返回来源和证据定位。

记忆读取结果必须以“背景数据”注入，不能改变系统策略、工具权限或用户身份范围。模型不得因为相似度高就把历史结论当成当前事实；当前事实仍需重新查询确定性分析工具。

### 5.3 整合和遗忘

任务结束后只提取候选记忆，经过 schema 校验和证据校验后再写入。异步整合任务负责去重、合并相同观察、处理时间冲突并从重复情节中提炼摘要；整合失败不影响本次回答。支持按用户删除个人会话、历史摘要、训练目标和实体事实；删除个人数据时同步删除向量索引和缓存。知识库的过期、撤回和版权状态独立处理。
