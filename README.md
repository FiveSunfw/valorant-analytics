...
**技术栈：** TypeScript、Node.js、RabbitMQ、PostgreSQL、Redis、Qdrant、Docker Compose
...
- 让同段位基准只统计满足授权、竞技模式和最小样本门槛的去标识化用户；让训练目标和已确认分析摘要作为用户上下文，而不是比赛证据。
...
> 当前仓库的长期训练记忆使用 PostgreSQL 实现，未接入 Mem0；简历中不把 Mem0 写成已落地组件。真实 Riot RSO 生产认证和线上 Qdrant/Embedding 部署属于上线配置，不影响本地 Demo 闭环。
...
- 训练记忆：PostgreSQL 保存用户确认的训练目标和分析摘要；分析结果可由用户确认后保存为摘要。
...
前置条件：Node.js 22、Docker Desktop，以及项目根目录未提交的 `.env`。真实模型需要配置 `DEEPSEEK_API_KEY`；没有密钥时会使用确定性模型完成本地流程测试。
