...
> 当前是本地可演示版本：使用脱敏 fixture、`deepseek-flash` 和本机 Docker 基础设施；尚未接入真实 Riot RSO 或 Match API 同步。
...
- 真实模型 Eval：15 个覆盖正常、空数据、小样本、权限与安全边界的 case；最近一次 `deepseek-flash` 结果为 **15/15**。
...
推荐的公开演示拓扑是：Cloudflare 托管 Web，独立的受保护 API 托管在可运行 Node/Fastify 且能连接 Postgres/Redis 的环境；Web 的 `API_BASE_URL` 指向该 API。真实 Riot RSO 上线前，还需把回调 URL 换成公网 HTTPS 地址并在 Riot 控制台登记。

## 产品边界

- 仅国际服、仅已完成竞技模式、仅当前授权玩家本人。
- 不保存 Riot 密码；密钥和 token 仅服务端保存。
- 不做 RAG、多 Agent、赛前对手侦察、实时对局指挥或作弊辅助。
