...
This is copied to `E:\valorant-video-ingestion` and is intentionally not served by the web app. It retrieves subtitles before ASR, falls back to the public Bilibili player API plus system `curl` when Python TLS is unavailable, retains JSON transcripts and selected frames privately, and removes temporary audio after each source.
